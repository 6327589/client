...
import { listPatentDisclosure, deletePatentDisclosure } from '@/api/analysis'
...
const getListData = async () => {
  try {
    ...
    // const res = await listPatentDisclosure({ listAll: true })

    console.log('searchForm.value', searchForm.value)
    const res = await listPatentDisclosure({ limit: 10000 })
    let list = res.items || []
    list = list.map(x => {
      return {
        id: x.ID,
        name: x.title,
        type: x.type === '3' ? '交底查新' : '未知',
        createdAt: format('minute', x.CreatedAt),
        updatedAt: format('minute', x.UpdatedAt),
      }
    })
...
const handleDelete = (row) => {
  confirmExecHandle('删除项目', '确认删除该项目吗？删除后，将无法继续访问该项目。', () => {
    return deletePatentDisclosure({ disclosure_id: row.id }).then(res => {
  ...
  confirmExecHandle(
    ...
    () => {
      const apis = selectedTable.value.map((x) =>
        deletePatentDisclosure({ disclosure_id: x.id }),
      )
        ...
        >
          ...
          <el-table-column prop="name" label="项目名称" min-width="300">
            <template v-slot="scope">
              <router-link :to="{name: 'Analysis_Disclosure_New', params: {id: scope.row.id}}" class="text-[var(--el-color-primary)] hover:underline cursor-pointer">
                {{ scope.row.name }}
              </router-link>
            </template>
          </el-table-column>
