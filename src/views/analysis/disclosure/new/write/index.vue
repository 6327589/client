...
import { getDisclosureWriteInfo, updatePatentDisclosureWrite } from '@/api/analysis'
...
const getData = () => {
  getDisclosureWriteInfo({ disclosure_id: disclosureId }).then((res) => {
    ...
    formData.value = {
      ...
      patent_title: res.data.patent_title,
      application_category: res.data.application_category || '发明',
      technical_field: res.data.technical_field,
      scheme_introduction: res.data.scheme_introduction,
      disadvantage: res.data.disadvantage,
      items: res.supplement_data.map(x => {
        return {
          a: x.technical_solution,
          b: x.beneficial_effect,
          c: x.specific_embodiment,
          sequence: x.sequence,
          opt: 'modify',
        }
      }),
      explanation_drawing: res.data.explanation_drawing,
      images: JSON.parse(res.data.attached_image || '[]'),
    }
...
const handleSave = () => {
  ...
  let supplement = formData.value.items.map((x, i) => {
    return {
      technical_solution: x.a,
      beneficial_effect: x.b,
      specific_embodiment: x.c,
      sequence: x.sequence,
      opt: x.opt,
    }
  })
  ...
  const body = {
    ...
    data: {
      applicant: formData.value.applicant, // 申请人
      inventor: formData.value.inventor, // 发明人
      phone: formData.value.phone, // 联系电话
      patent_title: formData.value.patent_title, // 专利名称
      application_category: formData.value.application_category, // 申请类别
      technical_field: formData.value.technical_field, // 技术领域
      scheme_introduction: formData.value.scheme_introduction, // 背景技术方案介绍
      disadvantage: formData.value.disadvantage, // 背景技术指出不足和缺点
      explanation_drawing: formData.value.explanation_drawing, // 附图说明
      attached_image: JSON.stringify(imgList), // 附图
    },
    supplement_data: supplement,
  }
  saveLoading.value = true
  updatePatentDisclosureWrite(body).then(res => {
  ...
  <div class="w-full h-full p-6 overflow-hidden">
    <div class="container mx-auto h-full flex flex-col overflow-hidden">
      <div class="flex flex-col w-full max-h-full overflow-hidden">
        <div class="flex-grow w-full flex bg-white rounded border border-light py-4 overflow-hidden">
          <!-- left -->
          <el-anchor class="flex-none h-full w-[180px] overflow-auto"
            ...
            >
              <el-form-item prop="applicant" id="form1">
                <div class="flex items-center gap-1">
                  <div class="flex-none w-5 h-5 flex items-center justify-center custom-bg-index2 text-white font-semibold text-xs">
                    01
                  </div>
...
高ISO数的图像在生成时通常会伴随着噪声的产生，去除噪声是用以提高图像的清晰度的常用手段。授权公告号为CN***B 的中国发明专利提供了一种去除图像噪声的方法，是使用平均滤波器来去除图像噪声。平均滤波器能通过对局部区域进行平均化处理，降低图像的变化幅度，从而达到提高图像清晰度的目的。"
        ...
        <div class="flex-none w-full pt-6 flex justify-end">
          <el-button class="" :loading="saveLoading" @click="handleSave">保存</el-button>
          <el-button class="" type="primary" :loading="submitLoading" @click="handleSubmit">开始检查</el-button>
        </div>
      </div>
    </div>
  </div>
