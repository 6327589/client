<script setup>
import { classnames, filter, format } from '@/utils'
import { onMounted, ref, computed, toRaw, watch } from 'vue'
import { semanticSearch, getPatentImages, exportPatentFile } from '@/api/search'
...
const getListData = async (pageQuery) => {
  ...
  const query = getQuery(searchLocalStore.searchFrom)
  const end = pageQuery.offset + pageQuery.limit

  const filterList = allList.filter((item, index) => {
    if (item.country_code && query.country_code && !query.country_code.includes(item.country_code)) {
      return false
    }
    if (item.type && query.type && !query.type.includes(item.type)) {
      return false
    }
    if (item.legal_status && query.legal_status && !query.legal_status.includes(item.legal_status)) {
      return false
    }
    if (item.current_status && query.current_status && !query.current_status.includes(item.current_status)) {
      return false
    }
    if (item.application_date && query.application_date) {
      const t = format('timestamp', item.application_date)
      const min = format('timestamp', query.application_date[0])
      const max = format('timestamp', query.application_date[1])
      if (t < min || t > max) {
        return false
      }
    }
    if (item.document_date && query.document_date) {
      const t = format('timestamp', item.document_date)
      const min = format('timestamp', query.document_date[0])
      const max = format('timestamp', query.document_date[1])
      if (t < min || t > max) {
        return false
      }
    }
  ...
  const list = filterList.filter((item, index) => {
    return index >= pageQuery.offset && index < end
  })
...
const handleSearch = async () => {
  const body = {
    search_key: searchLocalStore.semanticSearchValue,
  }
  tableLoading.value = true
  const res = await semanticSearch(body)
  let list = res.data || []
  let images = []
  ...
  list = list.map((x, i) => {
    return {
      ...
      score: x.score,
      scoreLabel: (x.score * 100).toFixed(2) + '%',
  ...
  searchLocalStore.clearSearchForm()
  allList = list
  tableRef.value.refreshTable()
  tableLoading.value = false
}

watch(() => searchLocalStore.semanticSearchValue, () => {
  handleSearch()
}, { immediate: true })
        ...
        <div class="w-full max-h-full px-6 py-4 flex flex-col">
          ...
          <div class="flex-grow w-full mt-2 bg-white p-4 overflow-hidden flex">
            <r-table ref="tableRef" tableName="Search_General_List" :getDataApi="getListData">
              <template #table="{ tableList }">
                <!-- 列表模式 -->
                <el-table v-if="showType === 'list'"
                  class="flex-grow" v-loading="tableLoading" :data="tableList"
                  @selection-change="handleSelectionChange">
                  <el-table-column type="selection" align="center" header-align="center" width="48"></el-table-column>
                  <el-table-column prop="scoreLabel" label="相似度" width="100"></el-table-column>
                  ...
                  <el-table-column prop="typeLabel" label="公开类型" width="120">
                    <template v-slot="scope">
                      <el-tag v-if="scope.row.type !== null" :type="scope.row.typeTagType">{{ scope.row.typeLabel
                        }}</el-tag>
                    </template>
                  </el-table-column>
