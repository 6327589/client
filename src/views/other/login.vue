<script setup>
import { loginByPhone, wxLogin, sendLoginCode } from 'api'
...
const loadCodeLoading = ref(true)
  ...
  <div class="w-screen h-screen flex flex-col" style="background: linear-gradient(110.94deg, #fdfffc 14.07%, #ecf6ea 92.42%);">
    ...
    <div class="flex-grow container mx-auto flex items-center">
      <div class="w-1/2 flex items-center justify-center">
        <div class="w-[400px]">
          <h1 class="text-black text-[36px] font-semibold">欢迎登录</h1>
          <div class="mt-6 flex items-center gap-10">
            <div :class="classnames(`py-2 font-semibold relative cursor-pointer
            `, loginType === 'phone' && 'text-[var(--el-color-primary)]'
            )"
              @click="loginType = 'phone'"
            >
              手机登录
              <div :class="classnames(`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-[var(--el-color-primary)] transition-all duration-300`,
                loginType === 'phone' ? 'w-full' : 'w-0',
              )"></div>
            </div>
            <div :class="classnames(`py-2 font-semibold relative cursor-pointer
            `, loginType === 'wx' && 'text-[var(--el-color-primary)]'
            )"
              @click="loginType = 'wx'"
            >
              微信登录
              <div :class="classnames(`absolute bottom-0 left-1/2 -translate-x-1/2 h-[2px] bg-[var(--el-color-primary)] transition-all duration-300`,
                loginType === 'wx' ? 'w-full' : 'w-0',
              )"></div>
            </div>
          </div>

          <!-- 手机登录 -->
          <el-form class="w-full mt-8" ref="formRef" :model="formData" :rules="formRule" v-if="loginType === 'phone'">
            ...
            <el-form-item class="mt-11">
              <el-button class="w-full" size="large" type="primary" @click="handleSubmit" :loading="submitLoading">登录</el-button>
            </el-form-item>
          </el-form>

          <!-- 微信登录 -->
          <div v-else class="w-full mt-8">
            <div class="w-[300px] h-[350px] overflow-hidden">
          ...
          <div class="mt-6 w-full">
            <span>没有账号吗?</span>
            <router-link  to="/register" class="ml-2 cursor-pointer hover:underline text-[var(--el-color-primary)]">注册新账号</router-link>
          </div>
        </div>
      </div>
      <div class="w-1/2">
        <img :src="getImgUrl('main.png')" class="w-full h-full object-contain" alt="login" />
      </div>
    </div>
  </div>
